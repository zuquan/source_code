...
class FunSetSuite extends FunSuite {
  ...
  test("intersect contains intersection elements") {
    new TestSets {
      ...
      val t = union(s2, s3)
      val i = intersect(s, t)
      assert(contains(i, 2), "insertsect 1")
      assert(!contains(i, 1), "insertsect 2")
      assert(!contains(i, 3), "insertsect 3")
    }
  }

  // test difference
  test("difference contains s-t") {
    new TestSets {
      val s = union(s1, s2)
      val t = union(s2, s3)
      val i = diff(s, t)
  ...
  // test exists
  test("exists satisfy at least one") {
    new TestSets {
      val s = union(s1, s2)
